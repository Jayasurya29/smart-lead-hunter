...
def upgrade() -> None:
    ...
    if _table_exists(conn, "potential_leads"):
        for col, idx in [
            ...
            (
                "lead_score",
                "CREATE INDEX IF NOT EXISTS idx_leads_score ON potential_leads(lead_score DESC)",
            ),
            (
                "created_at",
            ...
            (
                "hotel_name",
                "CREATE INDEX IF NOT EXISTS idx_leads_hotel_name_trgm ON potential_leads USING gin(hotel_name gin_trgm_ops)",
            ),
            (
                "brand",
                "CREATE INDEX IF NOT EXISTS idx_leads_brand ON potential_leads(brand)",
            ),
            (
                "state",
            ...
            # L-04 FIX: was "opening_status" which doesn't exist in schema
            (
                "location_type",
    ...
    # L-04 FIX: was "lead_sources" which doesn't exist — correct table is "sources"
...
def downgrade() -> None:
    for idx in [
        ...
        "idx_leads_score",
        "idx_leads_created",
        "idx_leads_hotel_name_trgm",
        "idx_leads_brand",
        "idx_leads_state",
        "idx_leads_country",
        "idx_leads_location_type",
        "idx_scrape_logs_source",
        "idx_scrape_logs_started",
        "idx_sources_active",
