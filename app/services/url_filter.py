...
class URLFilter:
    ...
    def __init__(self):
        ...
        for domain, rules in self.DOMAIN_RULES.items():
            self._domain_rules_compiled[domain] = {
                ...
                'require': [re.compile(p, re.IGNORECASE) for p in rules.get('require', [])],
                'allow': [re.compile(p, re.IGNORECASE) for p in rules.get('allow_patterns', [])]
            }
    ...
    def should_scrape(self, url: str, base_url: Optional[str] = None) -> URLFilterResult:
        """
        ...
        if not url or not url.startswith('http'):
            ...
            return URLFilterResult(
                url=url,
                should_scrape=False,
                reason="Invalid URL (not http/https)"
            )
...
def test_url_filter():
    ...
    test_urls = [
        # Should be BLOCKED
        ("https://www.hoteldive.com/selfservice/article-licensing/submit/?newspostUrl=...", False),
        ...
        # Should be ALLOWED
        ("https://www.hoteldive.com/news/ritz-carlton-hotel-development-indianapolis/809199/", True),
