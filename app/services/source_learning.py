...
class SourceLearning:
    ...
    )  # Patterns that produce leads
    junk_patterns: List[str] = field(
        default_factory=list
    )  # Patterns that never produce leads
...
class SourceLearningSystem:
    ...
    def record_batch_results(self, source_name: str, results: List[Dict]):
        """
    ...
    def _recalculate_metrics(self, learning: SourceLearning):
        ...
        if learning.total_urls_tested > 0:
            learning.lead_yield_rate = (
                learning.urls_with_leads / learning.total_urls_tested
            )

        # USA/Caribbean rate
        if learning.total_leads_found > 0:
            learning.usa_caribbean_rate = (
                learning.usa_caribbean_leads / learning.total_leads_found
            )
        ...
        if learning.total_urls_tested >= 10:
            learning.is_worth_scraping = (
                learning.lead_yield_rate >= 0.05
            )  # At least 5% yield
    ...
    def get_recommendations(self, source_name: str) -> Dict:
        ...
        if source_name not in self.learnings:
            return {"status": "untested", "message": "No data yet - run a test first"}
    ...
    def get_source_report(self, source_name: str) -> str:
        ...
        report.append("\n⭐ RECOMMENDATION:")
        report.append(
            f"   Worth scraping: {'✅ YES' if learning.is_worth_scraping else '❌ NO'}"
        )
    ...
    def get_all_sources_summary(self) -> str:
        ...
        if top_sources:
            ...
            for name, learning in top_sources:
                report.append(
                    f"   1. {name} - {learning.lead_yield_rate:.0%} yield, {learning.usa_caribbean_rate:.0%} relevant"
                )
...
class LearningOrchestrator:
    ...
    def _learn_from_leads(
        self, orchestrator, leads: List[Dict], sources: Optional[List[str]]
    ):
        ...
        for source_name in source_names:
            ...
            source_leads = [ld for ld in leads if ld.get("source_name") == source_name]
