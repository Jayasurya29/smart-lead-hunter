...
3. For opening_date use format "Month YYYY" (e.g. "September 2026"), "Q3 2026", "Late 2026", or "2027". NEVER just write "{current_year}" alone if the article does not specify a month or quarter — that is ambiguous and will be misinterpreted. If the article only says the year, write "Late {current_year}" or the next year instead.
   RELATIVE-DATE ANCHORING — CRITICAL: If the article uses a relative phrase like "this winter", "next spring", "this fall", "later this year", "early next year", "in the coming months", "this summer", etc., you MUST anchor it to the article's PUBLICATION DATE (not TODAY'S DATE). Example: article published December 30, 2025 says "set to open this winter" → winter 2025-2026 → extract as "Q1 2026" (Jan-Mar 2026), NOT "Late 2026". Example: article published July 2026 says "opening this fall" → fall 2026 → extract as "Fall 2026" or "Q4 2026". If the article has no publication date visible AND uses relative phrasing, leave opening_date empty rather than guessing.
...
7b. PUBLICATION DATE IS REQUIRED FOR RELATIVE DATES: Before interpreting any relative phrase ("this winter", "this spring", "later this year", "in the coming months", etc.), you MUST find the article's publication date (look in byline, dateline like "CHICAGO, IL (April 13, 2026)", URL slug, or article metadata). Use that date as the anchor. If the article says "opens this winter" and was published in December 2025, that means Q1 2026 — NOT "Late 2026". A 9-month error in opening_date extraction can misroute a lead into WARM/COOL when it should be URGENT/HOT or already-open. Return opening_date empty rather than guessing when publication date is not findable.
