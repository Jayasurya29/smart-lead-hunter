...
class ArticlePersonExtractor:
    ...
    PERSON_PATTERNS = [
        # "John Smith has been appointed General Manager"
        (
            r"([A-Z][a-z]+\s+[A-Z][a-z]+)\s+(?:has been|was|is|will be)\s+(?:appointed|named|hired|promoted|serve)\s+(?:as\s+)?(?:the\s+)?(.+?)(?:\s+of\s+the|\s+of\s+Four|\s+at\s+|\s+for\s+|,\s+over|,\s+who|\.)",
            "name_first",
        ),
        # "Sarah Johnson will serve as Director of Housekeeping"
        (
            r"([A-Z][a-z]+\s+[A-Z][a-z]+)\s+will\s+serve\s+as\s+(.+?)(?:,|\.|\s+over|\s+who)",
            "name_first",
        ),
        # "General Manager John Smith"
        (
            r"((?:General Manager|Executive Housekeeper|Director of [A-Za-z]+|Pre-Opening Director|Hotel Manager))\s+([A-Z][a-z]+\s+[A-Z][a-z]+)",
            "title_first",
        ),
        # "led by John Smith, General Manager"
        (
            r"(?:led by|headed by|managed by|under|overseen by)\s+([A-Z][a-z]+\s+[A-Z][a-z]+),?\s+(.+?)(?:\.|,|who)",
            "name_first",
        ),
        # "Contact: John Smith, Director of Sales"
        (
            r"[Cc]ontact:?\s*([A-Z][a-z]+\s+[A-Z][a-z]+),?\s+(.+?)(?:\s+at\s+|\s*@|\.)",
            "name_first",
        ),
    ]

    def extract_people(
        self, article_text: str, hotel_name: str = ""
                ...
                if len(groups) >= 2:
                    ...
                    if name and name not in seen_names and len(name) > 4:
                        ...
                        is_decision_maker = any(
                            t in title_lower for t in self.DECISION_MAKER_TITLES
                        )
                        ...
                        if any(
                            ...
                            t in title_lower for t in ["purchasing", "procurement"]
                        ):
                            relevance = 100
                        elif "general manager" in title_lower:
                            relevance = 90
                        elif (
                            "pre-opening" in title_lower or "task force" in title_lower
                        ):
                            relevance = 95
...
class TargetedContactFinder:
    ...
    async def find_contacts(
        ...
        if (
            self.hunter_key
            and domain
            and len(strategy.enriched_contacts) < 2
    ...
    async def _apollo_enrich_person(
        self, first_name: str, last_name: str, organization: str, title: str = ""
    ...
    def _generate_linkedin_searches(
        self, hotel_name: str, brand: str, location: str
    ) -> List[str]:
