"""
SMART LEAD HUNTER - LEAD SCORING SYSTEM (FIXED)
================================================
...
TIER3_UPPER_UPSCALE = [
    ...
    "jw marriott",
    "kimpton",
    "le meridien", "le méridien", "live aqua",
    "mgallery", "mr. c",
    "newbury boston",
    "omni",
    "pendry", "proper hotel",
...
# =============================================================================
# LOCATION SCORING (20 pts max) - FIXED VERSION
# =============================================================================
...
# =============================================================================
# INTERNATIONAL LOCATIONS TO SKIP - EXPLICIT LIST
# =============================================================================
INTERNATIONAL_SKIP = [
    # Canada
...
def get_location_score(city: str = None, state: str = None, country: str = None) -> Tuple[int, str]:
    """
    Score location based on market priority.
    
    Returns: (points, location_tier)
    - Florida: +20 pts
    - Caribbean: +15 pts
    - Strong US Markets: +15 pts
    - Other US: +10 pts
    - International: SKIP (return -1)
    """
    ...
    for intl_keyword in INTERNATIONAL_SKIP:
        if intl_keyword in location_text:
            return (-1, f"International - SKIP ({intl_keyword})")
    
    # ==========================================================================
    # CHECK US & CARIBBEAN MARKETS
    # ==========================================================================
    ...
    if country_lower and country_lower not in ["", "none", "null"]:
        ...
        return (-1, f"International - SKIP (country: {country})")
...
def should_skip_location(city: str = None, state: str = None, country: str = None) -> bool:
    ...
    score, _ = get_location_score(city, state, country)
    return score == -1
...
def get_timing_score(opening_date: str = None) -> Tuple[int, str]:
    """
    Score based on opening year.
    
    Returns: (points, timing_tier)
    - 2026: +25 pts (THIS YEAR - urgent!)
    - 2027: +18 pts (Next year - hot)
    - 2028: +12 pts (2 years out - warm)
    - 2029+: +6 pts (Far out - track)
    - Unknown: +4 pts
    """
    if not opening_date:
        return (4, "Unknown")
...
def calculate_lead_score(
    ...
    location_points, location_tier = get_location_score(city, state, country)
    ...
    # 3. TIMING (25 pts max)
    timing_points, timing_tier = get_timing_score(opening_date)
    ...
    if result["total_score"] >= 70:
        ...
        result["score_tier"] = "🌡️ WARM"
    elif result["total_score"] >= 30:
        result["score_tier"] = "❄️ COOL"
    else:
...
def format_score_breakdown(result: Dict) -> str:
    ...
    if not result["should_save"]:
        lines.append(f"⚠️  SKIP: {result['skip_reason']}")
        return "\n".join(lines)
    
    breakdown = result["breakdown"]
    
    lines.append(f"🏷️  Brand:     {breakdown['brand']['points']:>2} pts - {breakdown['brand']['tier']}")
    ...
    lines.append(f"🛏️  Rooms:     {breakdown['rooms']['points']:>2} pts - {breakdown['rooms']['tier']}")
...
if __name__ == "__main__":
    # Test international filtering
    test_cases = [
        ("Four Seasons Toronto", "Toronto", None, "Canada"),
        ("Four Seasons Bengaluru", "Bengaluru", None, "India"),
        ("Four Seasons Whistler", "Whistler", None, "Canada"),
        ("Four Seasons Milano", "Milan", None, "Italy"),
        ("Four Seasons Jacksonville", "Jacksonville", "Florida", "USA"),
        ("Aman Miami Beach", "Miami", "Florida", "USA"),
    ...
    for hotel, city, state, country in test_cases:
        score, tier = get_location_score(city, state, country)
        status = "❌ SKIP" if score == -1 else f"✅ {score} pts"
        print(f"{status:12} | {hotel:45} | {tier}")