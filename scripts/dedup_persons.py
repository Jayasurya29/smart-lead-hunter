"""
scripts/dedup_persons.py
========================
Group duplicate PEOPLE in lead_contacts under a shared person_id, so the same
human (e.g. "Robert De Niro" / "Robert Di Nero") stops appearing as two rows in
the Coverage card and directory.

WHAT IT DOES
------------
Fuzzy-matches lead_contacts that are the same person and assigns them a shared
`person_id` grouping key (migration 035). It NEVER deletes a row and never
touches the lead_id XOR existing_hotel_id invariant — person_id is an additive,
reversible tag. `--reset` clears it.

MATCH TIERS
-----------
  STRONG     same normalized email                      (auto-safe)
...
re-running never fragments earlier work.

USAGE (run from repo root)
--------------------------
  python scripts\\dedup_persons.py                 # dry-run, show all proposals
  python scripts\\dedup_persons.py --min-ratio 88  # stricter fuzzy threshold
  python scripts\\dedup_persons.py --strong-only   # email matches only
  python scripts\\dedup_persons.py --apply          # write person_id (STRONG+HIGH+REVIEW>=min)
  python scripts\\dedup_persons.py --apply --strong-only   # write only email matches
  python scripts\\dedup_persons.py --reset          # clear ALL person_id (undo)
  python scripts\\dedup_persons.py --selftest       # offline logic check, no DB
"""
...
def build_groups(rows: list[dict], min_ratio: float, strong_only: bool):
    ...
    # STRONG: same email (primary or secondary), regardless of org
    ...
    async with async_session() as s:
        res = await s.execute(
            select(
                ...
                LeadContact.name,
                LeadContact.email,
                LeadContact.secondary_email,
                LeadContact.organization,
                LeadContact.person_id,
            )
        )
...
async def _apply(uf: UF, comps: dict[int, list[dict]]) -> int:
    ...
    async with async_session() as s:
        for root, members in comps.items():
            ...
            await s.execute(
                update(LeadContact).where(LeadContact.id.in_(ids)).values(person_id=root)
            )
            n += len(ids)
        await s.commit()
    return n
...
async def _reset() -> int:
    ...
    async with async_session() as s:
        res = await s.execute(
            update(LeadContact)
            .where(LeadContact.person_id.is_not(None))
...
def _print_proposals(comps, link, min_ratio):
    ...
    for root in sorted(comps):
        ...
        # group's softest tier
        tiers = [
            link.get((min(a["id"], b["id"]), max(a["id"], b["id"])), (None, None))[0]
            for a in members for b in members if a["id"] < b["id"]
        ]
        ...
        if "REVIEW" in tiers:
            ...
            gtier = "PRESET"  # only a pre-existing person_id link — nothing new this run
        tier_counts[gtier] += 1
    ...
    print("\nReview above, then:  python scripts\\dedup_persons.py --apply")
...
def _selftest() -> int:
    rows = [
        ...
        {"id": 4, "name": "Mark Spencer", "email": "marks@noburestaurants.com", "secondary_email": "", "organization": "Nobu Restaurant Group", "person_id": None},
        {"id": 5, "name": "Mark Spencer", "email": "", "secondary_email": "marks@noburestaurants.com", "organization": "Some Other Co", "person_id": None},
    ...
    checks = [
        ...
        ("Spencer merged by email across orgs", has(4, 5)),
...
async def _amain(args) -> int:
    ...
    print(f"Loaded {len(rows)} lead_contacts.")
    ...
    if args.apply:
        ...
        print("Next: rebuild coverage-collapse so the card shows one row per person.")
        return 0
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fuzzy person-dedup for lead_contacts.")
    ...
    ap.add_argument("--strong-only", action="store_true", help="email matches only")
